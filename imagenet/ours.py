...
def init_down_from_cov(m, task, cur_cov, use_plora, lora_init_scale=3.0,
    ...
    ``s_weight=True`` designs the SHARED ``s_down`` from the MERGED BACKBONE WEIGHT instead of any
    data-derived signal: top-r input-side principal directions of ``linear_ours.weight``'s query/value
    block (equivalently its top-r right singular vectors). The weight is read off ``m`` directly --
    ``prepare_domain`` folds the previous domain's ``B@A`` into it before calling this, so it is
    the post-merge W (at the first boundary, the pristine source qkv). Needs no ``p_grad``.
    Off by default.

    ``s_histalign=True`` (shipped default for ours) designs the SHARED
    ``s_down`` from the HISTORY-ALIGNED entropy-gradient component ``U U^T g``, where ``U`` is past
    domains' input principal subspace (top eigvecs of ``prev`` covering ``private_history_proj_thr`` of
    the S^2 energy -- the SAME subspace pillar-3 removes for ``p_down``). So ``s_down`` spans
    ``span(U)`` while ``p_down`` spans its orthogonal complement: the shared / private split is
    exactly the history-projection split of one entropy gradient (As _|_ Ap), mirroring the
    history-aligned / history-residual decomposition. At the first boundary ``prev`` is 0 (no
    history) so it falls back to the raw-gradient top-r (same as ``use_shared_grad_init``). Needs ``p_grad``
    every boundary; takes precedence over ``use_shared_grad_init``. Off by default.
    """
    r = m.r
    ...
    if s_weight:
        # As = top-r input-side principal directions of the merged backbone's query/value block.
        # k rows are dropped exactly as in the gradient path (the ups only ever emit [query|value]).
        ...
        # As = top-r left singular vectors of the history-aligned query/value entropy gradient.
        # (U = past domains' input principal subspace, the SAME subspace pillar-3 removes for
        # p_down). As spans span(U), p_down spans its complement -> As _|_ Ap. prev = 0 at the
        # first boundary (k=0) -> no projection -> raw-gradient top-r fallback.
        projection_dim = m.qkv_dim
...
class _OursBase(nn.Module):
    ...
    def __init__(self, model, optimizer, steps=1, episodic=False, rank=32,
        ...
        # As init for the SHARED s_down from the MERGED BACKBONE WEIGHT: top-r input-side
        # principal directions of the post-merge query/value weight block. Needs no gradient.
        # Off by default.
        self.s_weight = s_weight
        # As init for the SHARED s_down from the HISTORY-ALIGNED entropy-gradient component
        # U U^T g (U = past-domain input principal subspace): As spans span(U), p_down spans its
        # complement, so As _|_ Ap and the shared/private split is one gradient's
        # history-projection split. Needs p_grad every boundary (like use_shared_grad_init); takes
        # precedence over use_shared_grad_init. Off by default; enabled by Ours.
    ...
    def prepare_domain(self, x):
        ...
        # use_shared_grad_init / s_histalign additionally need it at every boundary incl. domain 0 (their As
        # come from this gradient); s_weight needs it not at all, leaving this to use_private_grad_init alone.
